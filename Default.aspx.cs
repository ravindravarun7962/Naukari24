...
namespace Success24_Job_Portal
{
    public partial class Default : System.Web.UI.Page
    {
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                Response.Redirect(ResolveUrl("~/Jobs.aspx"));
                return;
            }
            ...
            if (string.IsNullOrWhiteSpace(location))
            {
                ...
                Response.Redirect(ResolveUrl("~/Jobseeker/Looking-for-" + keywordSlug));
                return;
            }
            ...
            {
                string citySlug = CreateSlug(location);
                Response.Redirect(ResolveUrl("~/" + citySlug + "/Jobseeker"));
                return;
            }

            // Both keyword and location
            string city = CreateSlug(location);
            string keywordText = CreateSlug(keyword);
            string url = "~/" + city + "/Jobseeker/Looking-for-" + keywordText;
        ...
        protected string GetLocation(object city,object state)
        {
            ...
            {
                return Server.HtmlEncode(cityText + ", " + stateText);
            }
        ...
        protected string GetExperience(object minValue,object maxValue)
        {
            ...
            if (minMonths > 0 && maxMonths > 0)
            {
                return FormatExperience(minMonths) + " - " + FormatExperience(maxMonths);
            }

            if (minMonths > 0)
            {
                return FormatExperience(minMonths) + "+";
            }

            return "Up to " + FormatExperience(maxMonths);
        }
        ...
        private string FormatExperience(int months)
        {
            ...
            decimal years = Math.Round(months / 12m,1);
            return years.ToString("0.#") + (years == 1? " year": " years");
        }
        ...
        private string FormatSalary(decimal salary)
        {
            if (salary >= 10000000)
            {
                return "₹" + (salary / 10000000m).ToString("0.##") + " Cr";
            }


            if (salary >= 100000)
            {
                return "₹" + (salary / 100000m).ToString("0.##") + " L";
            }

            if (salary >= 1000)
            {
                return "₹" + (salary / 1000m).ToString("0.##") + "K";
            }
        ...
        protected string GetPostedDate(object value)
        {
            ...
            return "Posted " + createdAt.ToString("dd MMM yyyy");
        }
